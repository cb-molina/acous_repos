...
def create_grid_pos(x_size, y_size, x_max, y_max):

    x_pos = np.zeros((y_size, x_size))
    y_pos = np.zeros((y_size, x_size))

    for x in range(x_size):
        y_pos[:,x] = np.linspace(0, y_max, y_size)
    
#    for x in range(x_size):
#        y_pos[:,x] = np.linspace(0, y_size, 5)

    for y in range(y_size):
        x_pos[y,:] = np.linspace(0, x_max, x_size)
...
x_max = 40
y_max = 5
x_pos, y_pos = create_grid_pos(x_size=x_size, y_size=y_size, x_max=x_max, y_max=y_max)
